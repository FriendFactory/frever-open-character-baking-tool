...
namespace UMA.AssetBundles
{
	...
	public class AssetBundleManager : MonoBehaviour
	{
        ...
        public static IReadOnlyDictionary<string, LoadedAssetBundle> LoadedAssetBundles => m_LoadedAssetBundles;
        ...
        public static bool UseDynamicIndexer = false;
	   ...
	   ///  any asset bundle without having to download it first. 
		...
		public static bool SimulateAssetBundleInEditor
		{
			get
			{
				...
				/*if (Application.isPlaying == false) // always simulate when out of play mode
					return true;*/ // Fenrir 04.03.2020 / if we set all flags to not simulate - then we won't simulate
		...
		{
          //  UnityEngine.Debug.Log("DWC GetLoadedAssetBundle " + assetBundleName);
			if (m_DownloadingErrors.TryGetValue(assetBundleName, out error))
			{
             //   UnityEngine.Debug.Log("DWC m_DownloadingErrors.TryGetValue(assetBundleName, out error) " + assetBundleName + " " + error);
				if (!error.StartsWith("-"))
				{
					...
					if (assetBundleName == Utility.GetPlatformName().ToLower() + "index")
					{
						...
						else
						{
                            ...
                            //SimulateOverride = true; // Fenrir 04.03.2020 / if we set flag to false - we need it
                        }
                    }
					else
#endif
             ...
             //   UnityEngine.Debug.Log("DWC GetLoadedAssetBundle return null" + assetBundleName);
				return null;
			}

          //  UnityEngine.Debug.Log("DWC GetLoadedAssetBundle no errors " + assetBundleName);
			LoadedAssetBundle bundle = null;
			m_LoadedAssetBundles.TryGetValue(assetBundleName, out bundle);
            if (bundle == null)
            {
              //  UnityEngine.Debug.Log("DWC bundle == null m_LoadedAssetBundles.TryGetValue(assetBundleName, out bundle); " + assetBundleName);
                return null;
            }


           // UnityEngine.Debug.Log("DWC GetLoadedAssetBundle no errors 2" + assetBundleName);
			// No dependencies are recorded, only the bundle itself is required.
			string[] dependencies = null;
			if (!m_Dependencies.TryGetValue(assetBundleName, out dependencies))
				return bundle;

           // UnityEngine.Debug.Log("DWC GetLoadedAssetBundle no errors 3" + assetBundleName);
          ...
          //  UnityEngine.Debug.Log("DWC return bundle" + assetBundleName);
			return bundle;
		}
		...
		static public AssetBundleLoadIndexOperation Initialize(string indexAssetBundleName, bool useJsonIndex, string jsonIndexUrl)
		{
            ...
            else //if we dont have a connection try to load a cached index otherwise trigger m_ConnectionChecker.ShowConnectionRequiredUI()
			{
				...
				else
				{
					...
					{
                      //  UnityEngine.Debug.Log("DWC add 2");
						m_UsingCachedIndex = true;
                        m_LoadedAssetBundles.Add(indexAssetBundleName.ToLower() + "index", new LoadedAssetBundle("CachedIndex")); //DWC not being added
					}
					m_InProgressOperations.Add(operation);
					return operation;
				}
		...
		{
            var indexName = Utility.GetPlatformName().ToLower() + "index";
            if (overrideBaseDownloadingURL != null && indexName != bundleName)
			{
		...
		static protected string RemapVariantName(string assetBundleName)
		{
			...
			for (int i = 0; i < bundlesWithVariant.Length; i++)
			{
				...
				int found = Array.IndexOf(m_ActiveVariants, curVariant);
		...
		static protected bool LoadAssetBundleInternal(string assetBundleToFind, bool isLoadingAssetBundleIndex = false, bool useJsonIndex = false, string jsonIndexUrl = "")
		{
			...
			else
			{
                ...
                string url = bundleBaseDownloadingURL;// + assetBundleToGet; // Fenrir 04.03.2020 / we have full path to bundle (named not as bundle what we looking for)
		...
		static private bool IsIndexBundle(string name)
		{
			string platformName = Utility.GetPlatformName().ToLower(); 
			return name.IndexOf(platformName + "index") != -1;
		}
		...
		/// Unloads all unused AssetBundles compressed data to free up memory.
		/// </summary>
		static public void UnloadAllAssetBundles(HashSet<string> ignoreSet, bool unloadAllLoadedObjects = false)
		{
#if UNITY_EDITOR
			// If we're in Editor simulation mode, we wont have actually loaded and bundles
			if (SimulateAssetBundleInEditor)
			...
			foreach (KeyValuePair<string, LoadedAssetBundle> kp in m_LoadedAssetBundles)
			{
				if (!IsIndexBundle(kp.Key) && !ignoreSet.Contains(kp.Key)) //dont try to unload the index...
				{
					bundlesToUnload.Add(kp.Key);
				}
			}
			foreach (string bundleName in bundlesToUnload)
			{
				UnloadAssetBundleInternal(bundleName, unloadAllLoadedObjects:unloadAllLoadedObjects);
		...
		public static void UnloadAssetBundleIndex(string platformName = "")
		{
			if (string.IsNullOrEmpty(platformName))
			{
				platformName = m_SessionIndexAssetBundleName.ToLower();
			}
			
			//we are also going to need to remove it from downloaded bundles otherwise it will get skipped
			m_LoadedAssetBundles.Remove(platformName + "index");
		...
		void ProcessFinishedOperation(AssetBundleLoadOperation operation)
		{
            ...
            if (download == null)
            {
              //  UnityEngine.Debug.Log("DWC download == null");
                return;  
            }
			...
			if (String.IsNullOrEmpty(download.error))
			{
			//	UnityEngine.Debug.Log("DWC [AssetBundleManager] processed downloaded bundle " + download.assetBundleName);
                if (!m_LoadedAssetBundles.ContainsKey(download.assetBundleName))
                {
               //     UnityEngine.Debug.Log("DWC Add 1");
				    m_LoadedAssetBundles.Add(download.assetBundleName, download.assetBundle); //DWC not being added here? 
                }
			}
			else
			{
              ...
              //  UnityEngine.Debug.Log("DWC 69  " + msg);
                if (!m_DownloadingErrors.ContainsKey(download.assetBundleName))
                {
