...
namespace UMA.CharacterSystem
{
    public class DynamicCharacterAvatar : UMAAvatarBase
    {
        ...
        public Dictionary<string, List<MeshHideAsset>> MeshHideDictionary { get { return _meshHideDictionary; } }
        private Dictionary<string, List<MeshHideAsset>> _meshHideDictionary = new Dictionary<string, List<MeshHideAsset>>();
        ...
        public bool BuildCharacterEnabled
        {
            ...
            set
            {
                _buildCharacterEnabled = value; return; // Fenrir / when buildCharacterEnabled there no sense to rebuild character with resetting Animator and other stuff
        ...
        public void ClearCachedData()
        {
            ...
            {
                generatedMaterial.materialFragments.Clear();
            }

            _meshHideDictionary.Clear();

            // Clear cached recipes in system
            var system = context.dynamicCharacterSystem as DynamicCharacterSystem;
            system.ClearCacheRecipes();
            
            // Clear mesh data left in the mesh combiner
            var generator = umaGenerator as UMAGeneratorBuiltin;
            if (generator != null)
            {
                var meshCombiner = generator.meshCombiner as UMADefaultMeshCombiner;
                if (meshCombiner != null)
                {
                    meshCombiner.ClearCombinedLists();
                }
            }

            // Clear mesh data left in the context
            context.slotLibrary.ReleaseLibrary();
        }
        ...
        public void SetAnimatorController(bool addAnimator = false)
        {
            ...
            if (controllerToUse != null)
            {
                ...
                {
                    if (thisAnimator.runtimeAnimatorController == null) // Fenrir 26.11.2020
                    {
                ...
                foreach (UMATextRecipe utr in WardrobeRecipes.Values)
                {
                    ...
                    if (utr.MeshHideAssets != null && !SuppressSlotsStrings.Contains(utr.wardrobeSlot))
                    {
                        foreach (MeshHideAsset meshHide in utr.MeshHideAssets)
                        {
                            ...
                            {
                                if (!_meshHideDictionary.ContainsKey(meshHide.asset.name))
                                {   //If this meshHide.asset isn't already in the dictionary, then let's add it and start a new list.
                                    _meshHideDictionary.Add(meshHide.asset.name, new List<MeshHideAsset>());
                                }

                                //If this meshHide.asset is already in the dictionary AND the meshHide isn't already in the list, then add it.
                                if (!_meshHideDictionary[meshHide.asset.name].Contains(meshHide))
                                {
                                    _meshHideDictionary[meshHide.asset.name].Add(meshHide);
                                }
                            }
                        }
                    }
        ...
        bool LoadCharacter(UMARecipeBase umaRecipe, List<UMAWardrobeRecipe> Replaces, params UMARecipeBase[] umaAdditionalSerializedRecipes)
        {
            ...
            foreach (SlotData sd in umaData.umaRecipe.slotDataList)
            {
                ...
                if (_meshHideDictionary.ContainsKey(sd.asset.name))
                {   //If this slotDataAsset is found in the MeshHideDictionary then we need to supply the SlotData with the bitArray.
                    sd.meshHideMask = MeshHideAsset.GenerateMask( _meshHideDictionary[sd.asset.name] );
                }
        ...
        public void Cleanup()
        {
            ...
            if(smr != null) Destroy(smr.sharedMesh);
        }
